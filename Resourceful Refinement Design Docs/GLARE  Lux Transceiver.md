...
The Lux Transceiver is the bridge between a [[GLARE Networks|GLARE network]] and any machine that wants to consume Lux without being a network node itself. Rather than making every Lux-hungry machine a targetable GLARE node, the transceiver docks against a machine's Lux-Socket face and pipes network Lux into it, keeping the connection logic in one reusable block.

## Construction & Placement

The Lux Transceiver is a directional block, which can face in any of the 6 directions. Like other GLARE nodes, it can be connected to a network, and supports a single link.
