...
These interfaces are the design-intent abstractions the framework is built around:

- **IGlareNode** — any block/BlockEntity (or held item) that can be a member of a GLARE network and be targeted by the connection tool.
- **IGlareEmitter** — a node that produces an amount of Lux (int >= 0) and may supply a Colour-Charge, contributing to the network's total Lux Capacity.
- **IGlareReceiver** — a node that allocates a portion of the network's Lux to do work, and carries an 'Operation Status' used to recover from Overload. Variable-Lux receivers broadcast a per-cycle Lux curve.

The Lux-Socket interface (see [[GLARE  Lux Transceiver]]) lets a non-node BlockEntity expose a GLARE interface on one of its faces for a transceiver to bind to.
...
See [[Telemetry Terminal GUI]] for the full terminal GUI specification.
...
See [[Remote Entanglement]] for the full depot and transporter specification.
