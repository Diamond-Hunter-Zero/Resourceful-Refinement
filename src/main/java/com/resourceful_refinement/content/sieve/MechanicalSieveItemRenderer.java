...
public class MechanicalSieveItemRenderer extends BlockEntityWithoutLevelRenderer {
    public static final MechanicalSieveItemRenderer INSTANCE = new MechanicalSieveItemRenderer();

    private MechanicalSieveCasingModel casingSingle;
    private MechanicalSieveCogModel cog;

    public MechanicalSieveItemRenderer() {
    ...
    private void prepareModels() {
        if (casingSingle != null) return;
        EntityModelSet models = Minecraft.getInstance().getEntityModels();
        this.casingSingle = new MechanicalSieveCasingModel(models.bakeLayer(MechanicalSieveLayers.CASING));
        this.cog = new MechanicalSieveCogModel(models.bakeLayer(MechanicalSieveLayers.COG));
    }
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext context, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
        ...
        // Render block model
        VertexConsumer vc = buffer.getBuffer(RenderType.entityCutout(MechanicalSieveRenderer.TEXTURE));
        casingSingle.render(ms, vc, light, overlay);

        // Render Top Cog (Inverted direction from middle, Half speed)
        // Raised by 2 pixels: -8/16f -> -10/16f
        ms.pushPose();
        ms.translate(0, -10 / 16f, 0);
        cog.render(ms, vc, light, overlay);
        ms.popPose();
